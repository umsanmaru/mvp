...
const rankUpdate = (result, uid, rank) => {
    if(result === 'win'){
        db.ref(`user_info/${uid}/rank`).set(Math.min(rank + 1, MAXRANK));
    }
    else if(result === 'lose'){
...
const currentUpdate = (uid, rank, result) => {
...
const EndPage = ({uid, gid, rank}) => {
    ...
    console.log(uid, gid, rank)
    
    const onReturn = () => {
        db.ref(`user_info/${uid}/status_object/status`).set("isWaiting");
    ...
    useEffect(() => {
        if (resultObj) {
            ...
            currentUpdate(uid, rank, result);
            totalUpdate(uid, result);
            db.ref(`game_info/${gid}/${rank}/result_object/${uid}/resultDone`)
                    .set(true);
        }    
    }, [resultObj])
